...
```{r}
data |> summarize(correlation = cor(GDP, DJIA, method="pearson"))
```
...
The DJIA and GDP have a very strong positive correlation, which is evident by this scatterplot/the correlation coefficient. The closer to 1, the stronger the relationship. This shows that when the DJIA increases, so does the GDP, or vice versa. They move in the same direction. 
