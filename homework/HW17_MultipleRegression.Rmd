...
**The variables voter turnout and percentage unemployed are the most strongly correlated, with a negative correlation. As voter turnout decreases, percentage unemployment increases, which would make sense. Another strongly correlated pair of variables would be voter turnout and percentage white, with a strong positive correlation. As voter turnout increases, so does the percentage white variable. All of the other combinations of variables seem correlated, but not as strong as the others mentioned above. There are, of course, outliers and a wider spread of points, indicated a weaker relationship. All of them seem to be linear, just with some being stronger than others. **
...
**The R-squared of 0.85 is very high, indicated that the model explains 85% of the variation in turnout which is strong, as well as the adjusted r-squared of 0.8023. The F-statistic p-value is also highly statistically significant. This would indicate a strong, well-fitting model. Income, median age and pct_white are significant predictors, each having p-values less than .05 while population, pct_bachelors_higher and pct_unemployed are insignificant as their p-values are greater than 0.05.**
...
**Model 3 with pct_white is significantly better than the others with an adjusted r-squared of 0.45 while the other two have adjusted r-squared values of around 0.2.**

```{r}
# Model 2
model2 <- lm(turnout ~ median_age , data = md_turnout)
summary(model2)
...
**This analysis could be used to cover an election or voter participation story by comparing those areas with higher incomes and with white populations with those with areas where the population is mostly unemployed and without a bachelors degree and ask the question as to why this is the case and what barriers are in place that prevent higher levels of voter turnout.**